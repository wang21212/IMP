...
每个新里程碑开始时，先做轻量意图校准：读 milestone-state 中该里程碑的目标，结合前序里程碑的实际完成现状，确认前提是否仍然成立、scope 是否需要调整。若无偏差则快速锁定进入执行；若标记为 `[需重评]` 或前提已变，则递归回 imp-intent 做完整意图循环。不盲目继续上次的计划，也不过度重复已确认的意图。
